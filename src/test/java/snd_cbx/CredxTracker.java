package snd_cbx;
	...
	public class CredxTracker {

		WebDriver driver;
		WebDriverWait wait;

		private static final String SSH_HOST = CredxTestData.SSH_HOST;
		private static final String SSH_USER = CredxTestData.SSH_USER;
		private static final String SSH_PASSWORD = CredxTestData.SSH_PASSWORD;
		private static final String EXPECTED_SCRIPT = CredxTestData.EXPECTED_SCRIPT;
...
//	        System.setProperty("webdriver.chrome.silentOutput", "true");
			...
			wait = new WebDriverWait(driver, Duration.ofSeconds(CredxTestData.UI_WAIT_SECONDS));
		...
		public void ValidateNotiInDealerTracker() throws Exception {
			...
			String transactionId = CredxTestData.TRANSACTION_ID;
...
//			subscriberTracker.click();
			...
			// Select qtqr service
			WebElement credxbtn = wait.until(ExpectedConditions.elementToBeClickable(
					By.xpath("//*[@id=\"rightSectionDiv\"]/section/div/div/div/div/div[2]/div[4]/ul/li[3]/button")));
			credxbtn.click();
			Thread.sleep(2000);
			...
			WebElement noti = wait.until(
					ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[@id=\"InfoModalbody\"]/div[8]/div[2]")));
...
//			/ Full actual notification text
		...
		private String fetchExpectedMessageViaSSH(String transactionId) {
		    ...
		    try {
		        JSch jsch = new JSch();
		        session = jsch.getSession(SSH_USER, SSH_HOST, CredxTestData.SSH_PORT);
		        session.setPassword(SSH_PASSWORD);
		        session.setConfig("StrictHostKeyChecking", "no");
		        session.connect(CredxTestData.SSH_TIMEOUT_MS);
